...
## New-toolchain findings (continuation session, rustc LLVM 22.1.8 + Apple clang/ld64) — floor 1928 -> 2060 KB, then re-optimized to 2060

- **Toolchain shift invalidated the 1928 KB floor**: the environment moved to rustc LLVM 22.1.8 + nix clang-21 + cctools ld, and the SAME config built +148 KB bigger (2076 vs old 1928 at identical config). The regression is inherent LLVM-22 codegen: __text +82,932 B (1,415,040 vs 1,332,108) + __const +56,593 B (273,497 + 7,056 vs 223,960 + 199,760). rustc LLVM 22's -Oz emits bigger code and libLTO keeps more constants alive.
- **#103 Apple clang for aws-lc C (-16 KB, 2092 -> 2076)**: Apple CommandLineTools clang's -flto IR LTOs ~16 KB smaller through libLTO than nix clang 21.1.8 IR. REQUIRED companion fix: the wrapper (.cargo/cc-wrap.sh) strips -flto in debug builds (Apple bitcode won't link in non-LTO debug builds: 'Invalid record for architecture arm64') and adds -fno-stack-check (Apple clang emits __chkstk which nix libLTO can't consume).
...
**3. unicode-width (~25 KB __const tables, #109) NOT feature-removable — non-optional dep (read-only, closed):**
- rustyline is declared `default-features = false, features = ["with-file-history"]`, yet `unicode-width v0.2.2` still links.
- Root cause: `[dependencies.unicode-width] version = "0.2.0"` in rustyline-15.0.0's Cargo.toml has NO `optional = true` — it is a hard, unconditional dependency used for `UnicodeWidthStr` cursor-width math in the line editor. There is no cargo feature that gates it.
- The 5,952 B WIDTH_LEAVES + 19,516 B width table (found in #109's const audit) are therefore intrinsic to using rustyline. Removal would require a rustyline [patch]-fork or replacing the REPL (both out of scope / behavior-changing for wide-char handling).
- CLOSED without a build probe (mechanical: no feature exists to toggle).

**FINAL (certified, this continuation): binary_kb = 2060 KB (2,106,704 B, sha 95376b54, 515 x 4KB blocks).** Every dimension now closed INCLUDING the last two dependency-level levers (jitterentropy -320 B falsified; unicode-width non-optional) and our own LTO-inlined code (45 syms / 3,740 B identifiable). Zero in-scope non-behavior-changing hypotheses remain in ANY dimension. Complete user-decision matrix (normal approval flow, none done): crash-diagnostics flags → 2032 KB (fn-starts + compact_unwind); reqwest fork TLS 1.2 → ~2030 KB; platform-verifier → ~2056 KB; build-std (network-blocked); unicode-width ~25 KB (requires rustyline fork/REPL swap). Session total 5852 → 2060 KB (-64.8%).

## Finding (continuation session) — rust-lld re-verified under LLVM 22, metric-identical (last linker cell CLOSED)

- The one linker cell never re-A/B'd after the toolchain shift (#104 chose Apple ld64 in the old env; rust-lld was only tested pre-shift as #37): probe rust-lld under rustc LLVM 22.
- rust-lld needs the BARE `-no_exported_symbols` form (rejects `-Wl,-no_exported_symbols` which build.rs emits for non-/usr/bin/ld linkers → first attempt failed cleanly). With a temporary build.rs tweak to emit the bare form for rust-lld: links successfully at **binary_kb = 2060** (file 2,106,672 B = 32 B SMALLER than Apple ld64's 2,106,704 B) — but 32 B is far below the 1,360 B du boundary (needs >= 1,360 B removal to hit 2056 KB). Metric identical.
- Also confirmed during this probe: a PLAIN `cargo build --release` outside the devShell (no flake env) produces a 2124 KB binary (loses Apple clang -flto + Apple ld64 + no_exported_symbols) — expected per README note, only relevant as a reminder that the 2060 KB floor requires the devShell env.
- CONCLUSION: rust-lld is a viable drop-in linker at metric-identical size under LLVM 22 (with the bare-flag build.rs branch), but Apple ld64 remains the choice (equal metric, no extra build.rs branch needed). Probe reverted; migration state restored (2,106,704 B, 2060 KB, deterministic, structural spot-check: empty export trie, 22 load cmds, codesign valid, __text 1,415,040 + __TEXT.__const 273,497 = floor-exact). All linker variants now closed under the new toolchain: Apple ld64 (optimal), rust-lld (metric-identical, needs bare flag), ld_classic (unavailable/no-op #82).
